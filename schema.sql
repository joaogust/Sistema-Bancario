-- 1. ESTRUTURA BÁSICA (Criação das Tabelas)
...
CREATE TABLE movimentacoes (
    ...
    CONSTRAINT ck_tipo_movimentacao CHECK(tipo IN('PIX ENVIADO', 'PIX RECEBIDO', 'TED ENVIADA', 'DEPOSITO', 'SAQUE'))
);

-- 2. LOG AUDITORIA (Criação da Tabela, Função e Trigger)
-- Tabela para rastrear alterações em clientes
CREATE TABLE log_auditoria (
    id_log BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    data_hora TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    usuario_acao VARCHAR(50) DEFAULT CURRENT_USER, -- Usuário do banco que fez a alteração
    tipo_acao CHAR(1) NOT NULL, -- 'I' (INSERT), 'U' (UPDATE), 'D' (DELETE)
...
-- Trigger: Ativar a função na tabela 'clientes'
CREATE TRIGGER tr_auditar_clientes
AFTER INSERT OR UPDATE OR DELETE ON clientes
FOR EACH ROW
EXECUTE FUNCTION auditar_alteracao_cliente();

-- 3. VIEWS (Consultas Simplificadas e Gerenciais)
-- VIEW 1: Extrato Consolidado (Para uso no dashboard do cliente)
...
SELECT
    ...
    m.tipo,
    CASE
        WHEN m.tipo IN ('DEPOSITO', 'PIX RECEBIDO') THEN 'ENTRADA'
        ELSE 'SAÍDA'
    END AS fluxo,
    CASE
        WHEN m.tipo = 'PIX ENVIADO' AND m.nome_destino IS NULL THEN CONCAT('PIX ENVIADO (Chave: ', m.chave_destino, ')')
        WHEN m.tipo = 'PIX ENVIADO' THEN CONCAT('PIX ENVIADO para ', m.nome_destino)
        WHEN m.tipo = 'PIX RECEBIDO' THEN 'PIX RECEBIDO'
        WHEN m.tipo = 'TED ENVIADA' THEN CONCAT('TED ENVIADA para ', m.nome_destino, ' (', m.instituicao_destino, ')')
        WHEN m.tipo = 'DEPOSITO' THEN 'Depósito em Conta'
        WHEN m.tipo = 'SAQUE' THEN 'Saque em Terminal'
        ELSE 'Transação Desconhecida'
    END AS descricao_completa
FROM movimentacoes m
ORDER BY m.data_hora DESC;

-- VIEW 2: Visão Gerencial de Clientes
...
SELECT
    ...
    (SELECT t.numero FROM telefones t WHERE t.id_cliente = c.id LIMIT 1) AS telefone_principal
FROM clientes c
JOIN profissoes p ON c.id_profissao = p.id
JOIN contas co ON c.id = co.id_cliente
LEFT JOIN enderecos e ON c.id = e.id_cliente;

-- 4. PROCEDURES (Funções de Execução Lógica)
-- PROCEDURE: Realizar Saque
...
BEGIN
    IF p_valor <= 0 THEN
        RAISE EXCEPTION 'O valor do saque deve ser positivo.';
    END IF;

    -- 1. Bloqueia a linha da conta para verificar e atualizar o saldo
    SELECT saldo INTO v_saldo_atual FROM contas
    WHERE id = p_id_conta FOR UPDATE;
    ...
    IF v_saldo_atual < p_valor THEN
        RAISE EXCEPTION 'Saldo insuficiente (Saldo: %)', v_saldo_atual;
    END IF;
